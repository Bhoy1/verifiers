...
class PokerTask(TaskSet):
    ...
    async def build_prompt(self, role: str, state: State) -> Messages:
        ...
        return [UserMessage(content=situation)]
    ...
    async def on_turn_complete(self, state: State) -> None:
        ...
        print(f"[poker] {actor_id}: {action['action']}" + (f" {action.get('amount', '')}" if action['action'] == 'raise' else ""))
    ...
    def _resolve_showdown(self, state: State):
        ...
        if len(active) == 1:
            ...
            print(f"[poker] {winner} wins (last standing), pot={sum(e['contributions'].values())}")
        else:
            ...
            for w in winners:
                e["chips"][w] += share
            print(f"[poker] showdown: winners={winners}, pot={pot}")
    ...
    async def on_game_end(self, state: State) -> None:
        ...
        print(f"[poker] game over: profits={e['profits']}")
...
def create_agents(
    ...
    trainable_players: list[int] | None = None,
    ...
    Args:
        ...
        strategy: Default strategy for all players
        trainable_players: List of 1-indexed player numbers that are trainable.
                          Default: all players are trainable.
        player_models: Optional dict mapping player id to model name.
                      e.g. {"player1": "qwen/qwen3-235b-...", "player2": "qwen/qwen3-30b-..."}
                      None = all use the default model from eval/training command.
    """
    if trainable_players is None:
        trainable_players = list(range(1, num_players + 1))
    ...
    for i in range(1, num_players + 1):
        ...
        agents[pid] = Agent(
            ...
            max_tokens=50,
            is_trainable=(i in trainable_players),
            model=player_models.get(pid),
        )
...
def load_environment(
    ...
    agents = create_agents(num_players, strategy=strategy, player_models=player_models)
